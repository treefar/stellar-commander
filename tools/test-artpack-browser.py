...
with sync_playwright() as p:
    ...
    page.goto("http://127.0.0.1:5833/", wait_until="networkidle")
    page.wait_for_function("window.__artpack && window.__game && Object.keys(window.__artpack.packs).length === 8 && window.__artpack.scenery['space-v1']")
    ...
    page.locator("#stage").screenshot(path=str(SHOTS / "artpack-title.png"))

    page.evaluate("Strat.init({skill:.5, turnLimit:30, seconds:60}); Game.state='strategy'")
    page.wait_for_timeout(120)
    page.locator("#stage").screenshot(path=str(SHOTS / "artpack-strategy.png"))
    ...
    page.locator("#stage").screenshot(path=str(SHOTS / "artpack-battle-move.png"))
    page.keyboard.up("ArrowRight")

    page.evaluate("Game.step=()=>{}; Battle.paused=false; Battle.player.fireT=4")
    page.wait_for_timeout(80)
    page.locator("#stage").screenshot(path=str(SHOTS / "artpack-battle-fire.png"))
    page.evaluate("Battle.player.fireT=0; Battle.player.meleeT=7")
    page.wait_for_timeout(80)
    page.locator("#stage").screenshot(path=str(SHOTS / "artpack-battle-melee.png"))
    browser.close()
